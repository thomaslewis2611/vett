      ...
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;">
        ...
        <tr><td style="padding:40px 32px 24px 32px;">
          ...
          <table role="presentation" cellpadding="0" cellspacing="0"><tr><td style="background:#2D6A4F;border-radius:999px;">
            <a href="https://vetthome.com/pricing" style="display:inline-block;padding:14px 24px;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;">Renew my Buyer Pass — £24.99 →</a>
          </td></tr></table>
        </td></tr>
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY ?? (globalThis as any).SUPABASE_SERVICE_ROLE_KEY;
        const resendKey = process.env.RESEND_API_KEY ?? (globalThis as any).RESEND_API_KEY;
        const cronSecret = process.env.CRON_SECRET ?? (globalThis as any).CRON_SECRET;
