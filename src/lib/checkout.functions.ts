...
function buildMagicLinkHtml(actionLink: string, opts: { heading: string; body: string; cta: string }): string {
  ...
  <div style="max-width:560px;margin:0 auto;padding:0 20px;">
    ...
    <div style="padding:24px 8px 0;text-align:center;"><p style="font-size:12px;color:#888780;margin:0;">© 2026 vett · vetthome.com · Every listing. Vetted. </p></div>
  </div>
</body></html>`;
}
...
function getStripe() {
  const key = process.env.STRIPE_SECRET_KEY ?? (globalThis as any).STRIPE_SECRET_KEY;
